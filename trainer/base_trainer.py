...
class BaseTrainer(object):
    def __init__(self, config, writer):
        ...
        if self.config.neptune:
            name = self.config[
                'note'] + '_' + self.config.source + '_' + self.config.target
            self.run = neptune.init(
                project='solacex/UniDA-Extension',
                name=name,
                source_files=[],
                capture_hardware_metrics=False)  #, mode="offline")
            self.run['config'] = self.config
            self.run['name'] = self.config[
                'note'] + '_' + self.config.source + '_' + self.config.target
        ...
        if self.config.task == 'imagenet-caltech':
            self.src_loader = get_cls_sep_dataset(self.config,
                                                  self.config.source,
                                                  self.config.source_classes,
                                                  batch_size=100,
                                                  test=True)
        else:
            self.src_loader = get_dataset(self.config,
                                          self.config.source,
                                          self.config.source_classes,
                                          batch_size=100,
                                          test=True)
        self.tgt_loader = get_dataset(self.config,
                                      self.config.target,
                                      self.config.target_classes,
                                      batch_size=100,
                                      test=True)
    ...
    def save_txt(self):
        with open(osp.join(self.config.snapshot, 'result.txt'), 'a') as f:
            f.write(self.config.source[:2] + '->' + self.config.target[:2] +
                    ...
                    str(self.acc_best_h) + ' ' + str(self.h_best_acc) + ' ' +
                    str(self.best_prec) + ' ' + str(self.best_recall) + '\n')
            f.write(self.config.source[:2] + '->' + self.config.target[:2] +
    ...
    def print_loss(self, iter):
        ...
        if self.config.neptune:
            for key in self.losses.keys():
                self.neptune_metric('train/' + key, self.losses[key].item(),
                                    False)
    ...
    def close_validate(self, i_iter):
        ...
        test_loader = get_dataset(self.config,
                                  self.config.target,
                                  self.config.target_classes,
                                  batch_size=100,
                                  test=True)
...
#           pred2 = pred2[:, common_index]
            ...
            label = torch.where(label >= self.config.num_classes,
                                torch.Tensor([self.config.num_classes]).cuda(),
                                label.float())
    ...
    def open_validate(self, i_iter):
        ...
        for _, batch in tqdm(enumerate(self.test_loader)):
            ...
            N = neck.shape[0]
            simi2cluster = self.cos_simi(F.normalize(neck, p=2, dim=-1),
                                         t_centers)
        ...
        for k, v in self.cluster_mapping.items():
            if v in self.global_label_set:
                map_mask += torch.where(memo_pred_all == k,
                                        torch.Tensor([1.0]).cuda().float(),
                                        map_mask.float())

        pred_label = torch.where(map_mask > 0, cls_pred_all, uk_null)

        gt_all = torch.where(gt_all >= self.config.num_classes,
                             torch.Tensor([uk_index]).cuda(), gt_all.float())
        ...
        bi_prec = metrics.precision_score(gt_binary,
                                          pred_binary,
                                          zero_division=0)
        self.neptune_metric('val/bi recall[center]', bi_rec)
        self.neptune_metric('val/bi prec[center]', bi_prec)

        k_acc = (acc * len(accs.avg) - accs.avg[uk_index]) / (len(accs.avg) -
                                                              1)
    ...
    def sklearn_kmeans(self, feat, num_centers, init=None):
        ...
        if init is not None:
            kmeans = KMeans(n_clusters=num_centers, init=init,
                            random_state=0).fit(feat.cpu().numpy())
        else:
        ...
        return torch.from_numpy(center).cuda(), torch.from_numpy(
            t_codes).cuda(), score
    ...
    def save_npy(self, name, tensor, iter=None):
        ...
        if self.config.transfer_all:
            name = '{}_{}_'.format(self.config.source,
